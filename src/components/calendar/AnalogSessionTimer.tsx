...
interface AnalogSessionTimerProps {
...
const AnalogSessionTimer = ({
  ...
  const [customDuration, setCustomDuration] = useState(sessionDuration);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const startX = centerX + (radius - markerLength) * Math.sin(angle);
      const startY = centerY - (radius - markerLength) * Math.cos(angle);
    ...
    if (active) {
      ...
      ctx.lineWidth = 5;
      ctx.strokeStyle =
        progress < 75 ? "#3b82f6" : progress < 90 ? "#f59e0b" : "#ef4444";
      ctx.stroke();
    }
  }, [remainingSeconds, active, progress]);
  ...
  const toggleTimer = () => {
    if (active) {
      ...
      const now = new Date();
      setSessionStart(now);
      setEndTime(addMinutes(now, customDuration));
      setElapsed(0);
      ...
      timerRef.current = window.setInterval(() => {
        setElapsed((prev) => {
          ...
          const remainingMins = (customDuration * 60 - newElapsed) / 60;
          ...
          if (newElapsed >= customDuration * 60) {
            if (timerRef.current) {
              clearInterval(timerRef.current);
              timerRef.current = null;
            }
  ...
  return (
    <Card className="w-full max-w-md bg-white shadow-md">
      ...
      <CardContent className="space-y-4">
        ...
        {/* Duration Input (only when inactive) */}
        {!active && (
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Input
                ...
                onChange={(e) =>
                  setCustomDuration(parseInt(e.target.value) || sessionDuration)
                }
                className="w-full"
              />
              <Button
                variant="outline"
                onClick={() => setCustomDuration(240)}
...
export default AnalogSessionTimer;
